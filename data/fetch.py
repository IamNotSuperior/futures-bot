...
# Candidate symbologies for "MES" on GLBX.MDP3. These resolve to very different
# datasets, so the estimate is run across all of them before committing.
CANDIDATES = [
    ("MES.c.0", "continuous", "front-month continuous contract"),
    ("MES.v.0", "continuous", "highest-volume continuous contract"),
    ("MES.FUT", "parent", "every MES contract month"),
    ("MES", "raw_symbol", "literal raw symbol (expected to match nothing)"),
]

def cache_path(symbol: str) -> Path:
    slug = symbol.replace(".", "_").lower()
    return PROJECT_ROOT / "data" / f"{slug}_ohlcv_1m_2024-09_2026-08.parquet"
...
def estimate(client: db.Historical, symbol: str, stype_in: str) -> dict:
...
def run_estimates(client: db.Historical) -> list[dict]:
    print(f"Cost estimate | {DATASET} | {SCHEMA} | {START} -> {END} (end exclusive)")
    print("-" * 78)
    results = []
    for symbol, stype_in, note in CANDIDATES:
        r = estimate(client, symbol, stype_in)
        r["note"] = note
        results.append(r)
        if "error" in r:
...
def pull(client: db.Historical, symbol: str, stype_in: str, out_path: Path) -> pd.DataFrame:
...
def main() -> int:
    ...
    if cost > MAX_COST_USD:
        print(
            f"\nSTOP: estimate ${cost:.4f} exceeds the ${MAX_COST_USD:.2f} cap. "
            "Nothing was downloaded."
        )
        return 1

    print(f"\nEstimate ${cost:.4f} is within the ${MAX_COST_USD:.2f} cap - proceeding.")
    pull(client, args.symbol, args.stype_in, cache_path(args.symbol))
    return 0
